...
const EVENT_HEADERS = ['id', 'title', 'date', 'team', 'note', 'created_at']
...
const WORKFORCE_SHEETS = [['workforce_ot', OT_HEADERS], ['workforce_manpower', MANPOWER_HEADERS], ['workforce_events', EVENT_HEADERS], ['workforce_ot_history', OT_HISTORY_HEADERS], ['workforce_ot_approvals', OT_APPROVAL_HEADERS]]
...
// กลุ่มพื้นเหลืองในไฟล์ต้นฉบับ (TOON/KED/MO) เป็นอีกหน่วยงาน ไม่ใช่บ้านล่าง จึงไม่ดึงเข้าปฏิทินนี้
const PERSON_MAP = { TANG: ['แตง', 'คนแพ็ก'], PANG: ['แป้ง', 'คนแพ็ก'], FAH: ['ฟ้า', 'คนแพ็ก'], MII: ['มี่', 'คนแพ็ก'], PANID: ['ป้านิด', 'พาร์ทไทม์'], MAPRANG: ['มะปราง', 'อื่น ๆ'], ATOM: ['อะตอม', 'อื่น ๆ'], BAS: ['บาส', 'อื่น ๆ'], NEOY: ['เนย', 'อื่น ๆ'] }
const MONTH_BY_TAB = { JAN: '01', FEB: '02', MAR: '03', APR: '04', MAY: '05', JUN: '06', JUL: '07', AUG: '08', SEP: '09', OCT: '10', NOV: '11', DEC: '12' }

export function parseManpowerWorkbook(buffer) {
  const wb = XLSX.read(buffer, { type: 'buffer' }); const result = []
    ...
    for (let i = 0; i < dateRows.length; i++) {
      ...
      for (let c = 0; c < 7; c++) {
        ...
        for (let rr = r + 1; rr < end; rr++) {
          ...
          const code = raw.toUpperCase().match(/^[A-Z]+/)?.[0]; const person = PERSON_MAP[code]; if (!person) continue
...
async function opWorkforce(req, res) {
  if (req.method === 'GET' && String(req.query.sourceOnly || '') === '1') {
    try {
      ...
      return res.status(200).json({ success: true, sourceManpower, sourceUpdatedAt: new Date().toISOString() })
  ...
  if (req.method === 'GET') {
    ...
    const [rows, manpower, events, history, approvals] = values.map((range) => rowsToObjects(range.values || []))
    let sourceManpower = []
    try { if (process.env.MANPOWER_FILE_ID) sourceManpower = parseManpowerWorkbook(await downloadDriveFile(process.env.MANPOWER_FILE_ID)) } catch (e) { console.error('manpower source:', e.message) }
    res.setHeader('Cache-Control', cacheable('public, s-maxage=20, stale-while-revalidate=60'))
    const data = { success: true, rows: rows.sort((a, b) => String(b.date).localeCompare(String(a.date))), manpower, sourceManpower, events, history, approvals, sourceUpdatedAt: new Date().toISOString() }
  ...
  if (action === 'create-plan') {
    ...
    if (process.env.MANPOWER_FILE_ID) {
      try {
        const dayManpower = parseManpowerWorkbook(await downloadDriveFile(process.env.MANPOWER_FILE_ID)).filter((r) => r.date === body.date)
  ...
  if (action === 'update-plan') {
    ...
    if (changedRows.length) await appendRows('workforce_ot_history', changedRows.map((row, index) => { const u = updateMap.get(String(row.id)); return [`hist-${Date.now()}-${index}`, row.id, row.date, row.employee, row.planned_start, row.planned_end, u.planned_start, u.planned_end, row.note || '', u.note ?? row.note ?? '', changedAt, body.changed_by || 'Boss'] }))
  ...
  if (action === 'approve-actual-month') {
    ...
    const current = await getSheet('workforce_ot_approvals'); const now = new Date().toISOString(); const id = `approve-${body.month}-${body.employee}`
    const record = { id, month: body.month, employee: body.employee, actual_minutes: Math.round(Number(body.actual_minutes)), approved_at: now, approved_by: body.approved_by || 'Boss' }
    const found = current.some((r) => r.month === body.month && r.employee === body.employee)
    const next = found ? current.map((r) => r.month === body.month && r.employee === body.employee ? record : r) : [...current, record]
    await overwriteSheet('workforce_ot_approvals', OT_APPROVAL_HEADERS, next.map((r) => OT_APPROVAL_HEADERS.map((h) => r[h] ?? '')))
    clearWorkforceCache()
  ...
  if (action === 'create-event') {
    ...
    await appendRows('workforce_events', [[`event-${Date.now()}`, body.title, body.date, body.team || 'ทุกทีม', body.note || '', new Date().toISOString()]])
    clearWorkforceCache()
